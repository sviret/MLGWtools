...
Central class for LSTM_CNN denoising network generation
...
class CNN_LSTM():
    ...
    def LSTM_denoiser(self,LSTM_layers=1,LSTM_neurons=10):

        self.__units = LSTM_neurons # Number of neurons (100 in the paper)
        mode='concat'
        size=self.__npts
        self.__feature=4
        ...
        self.inputs=[]
        self.outputs=[]
        #fc_wiring = wirings.AutoNCP(30, 4)
        ...
        with tf.name_scope('CNN_LSTM') as scope:
            ...
            for i in range(LSTM_layers):
                x=layers.Bidirectional(layers.LSTM(self.__units, return_sequences=True), name=f"BiLSTM_layer_{i+1}", merge_mode=mode)(x)
